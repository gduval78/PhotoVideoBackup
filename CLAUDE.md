...
**Never bind a locale-formatted numeric `TextField` directly to `@AppStorage`.** `TextField(value: $storedInt, format: .number)` reads the **environment locale** for both formatting and parsing. Because the root injects `.environment(\.locale, languageManager.currentLocale)` and that locale **follows the system language** in "System Default" mode, a system-language change re-renders and re-parses the field, which can write a spurious value into the store (field observed: an "Unlimited" limit turning into `6` after a language switch). A non-optional `Int` also can't represent an empty/"unlimited" state — `0` formats to `"0"` and hides the placeholder. **Fix pattern** (see the "Max files per session" field in `SettingsView`): edit through a `@State private var` **String** buffer, initialise it in the view's load/onAppear (`0 → ""` so the placeholder shows), and commit explicitly (on focus loss / a keyboard **Done** button / `onDisappear`) via a **pure, locale-independent** parser that keeps digits only (`SettingsView.normalizedFileLimit(from:)` — empty/zero/non-numeric ⇒ 0). Unit-tested by `FileLimitFieldTests` (grouping separators like `"1,000"` / `"1 000"` / `"1.000"` must all parse to `1000`).
...
Both engines apply a 4-level cascade per file to decide whether to copy:
...
3. **Same-folder twin check** — `existingLocalTwinPath(inFolder:excludingName:size:sourceSHA256:)` (in `BackupTarget.swift`) scans the *actual* destination folder for a byte-identical file under a **different name**, per local target. Size is the free discriminator (a `stat`); SHA-256 is computed only for a same-size candidate and is the final judge. This closes the gap the index cannot see: a twin left by an **older app version, a Finder copy, or any other means** is never in `IndexStore`, so levels 1–2 miss it and a differently-named identical file (`X.mp4` / `X-1.mp4`) gets re-copied. A covered target is dropped from the copy set and the twin's path recorded, which **backfills the SHA index** so the next run skips via the cheap level-2 path. Local targets only (an SMB folder would need a download-to-hash per candidate). Fixes the field report of duplicated `Insta360LunaUltra.mp4` / `Insta360LunaUltra-1.mp4`. `FileCopyEngine` runs it before copying (source SHA already computed); `PHBackupEngine` runs it on the streamed path *after* writing (the asset SHA is only known once bytes flow) by adding any folder twin to `priorPaths` before the `coveredDestinationPaths` roll-back. Pure function, unit-tested by `TwinDedupTests` (size-collision-but-different-content must **not** match; a file must not be its own twin).
4. **Copy** — `streamCopy` writes to missing destinations using the precomputed SHA-256 (no double-read of source). SHA-256 is persisted in `IndexedFile` for future dedup.

**Why size-then-hash, not hash-everything.** Getting a file's size is O(1) (`URLResourceValues.fileSizeKey` / `attributesOfItem` — a `stat`, no bytes read); a SHA-256 requires a full read. So the folder scan pays only a directory listing in the common case and reads bytes only on a genuine size collision (almost always an actual duplicate). Weakening the hash to a cheap checksum (`sum`/CRC32) would **not** help — the cost is the I/O of reading the candidate, not the hash CPU — and would forfeit collision safety, so SHA-256 stays the judge.
...
|---------|-------|------------|---------|-------------|
| 2.5.1   | 47    | 2026-09-19 | Bug fix | **Three fixes around the "Max files per session" limit — the engine limit logic itself was never wrong.** **(1) The Settings field could not be validated.** *Max files per session* used a number-pad field with no way to dismiss the keyboard, so a typed value was never committed. A keyboard **Done** button now confirms it; an empty field means **Unlimited**. **(2) "Unlimited" could turn into a stray number after changing the phone's language.** The field was a locale-formatted number bound straight to storage; because the app's language override drives the on-screen locale, a system-language change could re-parse the field into a wrong value (a limit of "Unlimited" was seen becoming `6`), and a stored `0` never showed the "Unlimited" placeholder. The field is now a plain text buffer committed through a locale-independent parser — empty/zero/non-numeric all mean Unlimited — so language changes can't corrupt it. **(3) A partial backup was always reported as "file limit reached", even when it wasn't.** A backup stops early for three different reasons — the file limit, a destination that disconnected mid-run, or the user tapping Stop — but the completion **notification** and the **Report** hard-coded the "file limit reached" wording for all three. So a run that stopped because the SSD/NAS disconnected (e.g. at file 149, with the limit set to Unlimited) was mislabelled as a limit. Each cause now shows its own message: a disconnection reads **"destination disconnected — reconnect and run again"**, a user stop reads **"stopped — run again to continue"**, and only a real limit says "file limit reached". Implemented with a persisted `PartialReason` on `BackupSession` (lightweight migration; old sessions read as before), classified by a pure `DashboardViewModel.partialReason(...)`. Non-regression tests: `FileLimitFieldTests` (locale-independent parsing — grouping separators like `1,000`/`1 000`/`1.000` all parse to `1000`), `PartialReasonTests` (a disconnection/cancel is never classified as a file limit), and `BackupFromSDCardTests.test_fileLimit_skippedFilesDoNotCountAgainstLimit` (already-present files don't consume the limit budget). See also the new CLAUDE.md note on never binding a locale-formatted `TextField` directly to `@AppStorage`. |
...
- `existingLocalTwinPath(inFolder:excludingName:size:sourceSHA256:)` (`BackupTarget.swift`) — the same-folder twin scan (cascade level 3). Pure filesystem read against a temp folder; `TwinDedupTests` locks that a same-size-but-different-content file is not matched and that a file is never its own twin.
