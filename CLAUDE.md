...
- **Handles open independently (fixed after the 3-destination report).** `openDestinationHandles` opens one handle per local destination and drops any whose volume is gone (SSD unplugged) into a `disconnected` list instead of aborting the rest — losing the SSD must not stop an independent iCloud Drive destination. Previously the setup loop was all-or-nothing, so after the SSD disconnected, every subsequent file threw at setup and iCloud silently stopped too. A creation failure on a volume that is *still reachable* is a real error and still rethrows. Setup-disconnected destinations are merged with mid-write disconnections into the `StreamResult`. `DestinationIndependenceTests` covers this (including SSD-in-slot-1, the field ordering).
